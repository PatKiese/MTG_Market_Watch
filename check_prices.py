...
def get_latest_id(input_engine):
    ...
    tmp_df = read_from_db(tmp_query, input_engine)
...
def draw_price_time_series(file_path, input_engine, item_id):
    """Draw pricing time series corresponding to item_id."""
    df_time_series = fetch_table_from_id(input_engine=input_engine, input_id=item_id)
...
def create_sql_query_search_biggest_price_increase(currency="usd", mtg_format="commander", asc_desc="asc",
...
if __name__ == '__main__':
    ...
    latest_id = get_latest_id(engine)
    last_week_id = get_last_week_id(engine)
    for tmp_format in mtg_formats:
        # Create SQL query for each relevant MtG format
        sql_query = create_sql_query_search_biggest_price_increase(currency="usd", mtg_format=tmp_format,
